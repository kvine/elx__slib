defmodule  Com.Util do
    ...
    @doc """
        add key if key is not in map
    """
    def check_map_key(item,key,value) do 
         case Map.has_key?(item,key) do 
            true ->
                item
            false -> 
    ...
    @doc """
        items_weights: =[{id0,weight0},{id1,weight1}.....]
        sum_weight is not 0
        test:
        items_weight=[{0,100},{1,200},{2,300}]
        items_weight=[{0,1},{1,0},{2,0}]
        Com.Util.get_id_by_weight(items_weight)
    """
    
    def get_id_by_weight(items_weight) do 
        weights = :lists.foldr(fn(x,acc)-> 
                                {_id,w}=x
                                [w|acc]
                            end,[],items_weight)
        sum_weight=:lists.sum(weights)
        probs= for i <- weights do 
                    i/sum_weight
                end
        loc_id= Random.Util.get_locid_by_probs(probs) + 1
        {id,_weight}=:lists.nth(loc_id,items_weight)
        id
    end
